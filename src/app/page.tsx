...
function Portrait({ name, note, src }: { name: string; note: string; src?: string }) {
  return <div className={styles.portrait}>{src ? <Image src={src} alt={`Retrato de ${name}`} fill quality={95} sizes="(min-width: 720px) 320px, 210px" /> : <><span>Foto pendiente</span><strong>{name[0]}</strong><small>{note}</small></>}</div>;
}
...
export default function HomePage() {
  ...
  return <><main className={styles.home}>
    <section className={styles.hero} aria-labelledby="hero-title">
      <Image className={styles.heroBackdrop} src="/images/home/hero/Hero.png" alt="Camper estacionada frente al mar al atardecer, con una power station sobre una mesa exterior" fill priority unoptimized sizes="100vw" />
      <div className={`container ${styles.heroGrid}`}><div className={styles.heroCopy}><p className={styles.folio}>Cuaderno de viaje · 01</p><h1 id="hero-title">Nos conocimos <em>viajando.</em></h1><p className={styles.heroStatement}>Carga Nómada nació discutiendo sobre baterías.</p><p className={styles.lead}>Cinco formas de viajar acabaron enseñándonos que una misma power station puede ser perfecta para uno y un error para otro.</p><a className={styles.storyLink} href="#origen">Conoce nuestra historia <span aria-hidden="true">↓</span></a></div></div>
      <p className={styles.heroCaption}>Algún lugar entre una ruta,<br />una sobremesa y una batería al 12%.</p>
    </section>
    ...
    <section className={styles.chapters} aria-labelledby="chapters-title"><div className={`container ${styles.chapterHeading}`}><p className={styles.folio}>Cinco miradas, una historia</p><h2 id="chapters-title">La energía cambia cuando cambia el viaje.</h2></div>{chapters.map((chapter) => { const persona = bySlug[chapter.slug]; return <article className={styles.chapter} key={chapter.slug}><div className={`container ${styles.chapterGrid}`}><div className={styles.chapterVisual}><Portrait name={persona.name} note={chapter.number} src={persona.avatar} /><span>{persona.label}</span></div><div className={styles.chapterCopy}><p className={styles.folio}>{chapter.kicker}</p><h3>{chapter.title}</h3><p>{chapter.text}</p><blockquote>“{chapter.quote}” <cite>— {persona.name}</cite></blockquote><Link href={chapter.href}>{chapter.link} <span aria-hidden="true">→</span></Link></div></div></article>; })}</section>
