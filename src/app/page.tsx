...
export default function HomePage() {
  ...
  return <><main className={styles.home}>
    ...
    <section className={`${styles.section} ${styles.featured}`} aria-labelledby="featured-title"><div className={`container ${styles.featuredGrid}`}><div><p className={styles.kicker}>En portada · Review publicada</p><h2 id="featured-title">Análisis destacado</h2><p className={styles.sectionIntro}>La F2400 analizada para una caravana familiar, separando ficha oficial, estimaciones y criterio editorial.</p></div><article className={styles.featuredCard}><div className={styles.mediaPlaceholder} role="img" aria-label={`Imagen propia o autorizada de ${featuredProduct.name}, pendiente de incorporar`}><span>Imagen autorizada pendiente</span><b>{featuredProduct.model}<br />—</b></div><div className={styles.featuredCopy}><p className={styles.meta}>Power station · Review editorial</p><h3>{featuredProduct.name}</h3><dl className={styles.specList}><div><dt>Capacidad</dt><dd>{featuredProduct.capacityWh} Wh</dd></div><div><dt>Potencia continua</dt><dd>{featuredProduct.continuousOutputWatts} W</dd></div><div><dt>Batería</dt><dd>{featuredProduct.battery.chemistry}</dd></div><div><dt>Peso</dt><dd>{featuredProduct.weightKg} kg</dd></div></dl><p>Capacidad, conexiones y autonomía estimada para varios consumos familiares, sin atribuir pruebas físicas inexistentes.</p><Link href="/reviews/fossibot-f2400-review">Leer la review completa <span aria-hidden="true">→</span></Link></div></article></div></section>

    <section className={styles.section} aria-labelledby="reviews-title"><div className="container"><div className={styles.headingRow}><div><p className={styles.kicker}>Archivo · Publicadas</p><h2 id="reviews-title">Últimas reviews</h2></div><Link href="/reviews">Ver todas las reviews <span aria-hidden="true">→</span></Link></div><div className={styles.reviewGrid}>{reviews.slice(0, 3).map((review, index) => { const product = getProductById(review.productId); return <article className={styles.reviewCard} key={review.id}><div className={styles.thumb} aria-hidden="true"><span>0{index + 1}</span></div><p className={styles.meta}>{product?.name} · Análisis editorial</p><h3>{review.h1}</h3><p>{review.excerpt}</p><Link href={`/reviews/${review.slug}`}>Leer la review <span aria-hidden="true">→</span></Link></article>; })}</div></div></section>
